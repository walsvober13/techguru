...
document.addEventListener("DOMContentLoaded", () => {
  ...
  async function startVoiceRecording() {
    try {
      ...
      mediaRecorder.onstop = async () => {
        ...
        addMessage(
          "🎤 Voice message recorded (speech-to-text integration pending)",
          "user",
        );
  ...
  const startResize = (e, direction) => {
    ...
    const usingFixedPosition =
      containerStyle.position === "fixed" && containerStyle.left !== "auto";
  ...
  const resize = (e) => {
    ...
    if (usingFixedPosition) {
      ...
      maxAllowedHeight = Math.min(
        maxAllowedHeight,
        window.innerHeight - currentTop - 20,
      ); // 20px bottom padding
    }
    ...
    if (resizeDirection.includes("e")) {
      ...
      if (usingFixedPosition) {
        const widthDiff =
          Math.max(MIN_WIDTH, Math.min(MAX_WIDTH, newWidth)) - resizeStartWidth;
    ...
    if (resizeDirection.includes("s")) {
      ...
      if (usingFixedPosition) {
        const heightDiff =
          Math.max(MIN_HEIGHT, Math.min(maxAllowedHeight, newHeight)) -
          resizeStartHeight;
    ...
    const constrainedHeight = Math.max(
      MIN_HEIGHT,
      Math.min(maxAllowedHeight, newHeight),
    );
  ...
  const addMessage = (content, role) => {
    ...
    if (role === "assistant") {
      ...
      thumbsUp.addEventListener("click", () =>
        handleReaction(thumbsUp, "helpful"),
      );
      ...
      thumbsDown.addEventListener("click", () =>
        handleReaction(thumbsDown, "not-helpful"),
      );
  ...
  if (chatClear) {
    chatClear.addEventListener("click", () => {
      ...
      // Add welcome message
      addMessage(
        "Hi! I'm the TechGuru AI assistant. How can I help you today?",
        "assistant",
      );
    });
  }
  ...
  if (chatSuggestions) {
    const suggestionChips =
      chatSuggestions.querySelectorAll(".suggestion-chip");
